...
% Ipe = imread('PET.png');
%%找出指定目录下的所有指定文件
path1 = 'D:\matlab\my\2DMRI\MCIMRI\';
path2 = 'D:\matlab\my\2DPET\MCIPET\';
fileExt = '*.png';
...
len2 = size(files2,1);
for p=1:len1
    fileName1 = strcat(path1,files1(p,1).name)
    I = imread( fileName1 ); 
    Imr = imresize(I,[224 NaN]);
    for q = 1:len2
        if q == p
            disp(q);    
            fileName2 = strcat(path2,files2(q,1).name)
            J = imread( fileName2 );  
            Ipe = imresize(J,[224 NaN]);
          ...
          %% Initialize Parameters for PCNN
            ...
            %figure,imshow(FF);
            imwrite(FF,['D:\matlab\my\fusion\MCI\','MCI',num2str(p),'.png'])
         end
     end
end
disp('=======end=============')
...
% Imr = imread('ADMRI_001.png');
% Ipe = imread('ADPET_1.png');
% Imr = imresize(Imr,[224 NaN]);
% Ipe = imresize(Ipe,[224 NaN]);
